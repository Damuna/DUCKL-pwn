...
usage(){
    echo -e "\n[+] USAGE"
    echo -e "bloodypwn --dc [DC_FQDN] --ns [DC_IP] --no-gather                             (Skip Ingestion, pwn previously gathered ACLs)"
    echo -e "bloodypwn -u [USER] --dc [DC_FQDN] --ns [DC_IP] [-p PASSWORD] [-H HASH] [-k]  (Gathers LDAP Data as USER, pwn collected ACLs)"
    exit 1
}
...
if $NO_GATHER; then
    ...
    if [ -z "$DC_FQDN" ] || [ -z "$DC_IP" ]; then
        echo "[-] ERROR: --no-gather requires both --dc and --ns"
        usage
        exit 1
    fi
else
    ...
    if [ -z "$USERNAME" ] || [ -z "$DC_FQDN" ] || [ -z "$DC_IP" ]; then
        echo "[-] ERROR: You must provide -u, --dc, and --ns when gathering"
        usage
        exit 1
    else
